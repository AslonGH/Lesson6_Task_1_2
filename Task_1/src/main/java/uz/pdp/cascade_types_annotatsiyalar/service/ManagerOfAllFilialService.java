...
@Service
public class ManagerOfAllFilialService {
    ...
    public ApiResponse registerManager(UserRegisterDto managerDto){
       ...
       // manager.setFilial(optionalFilial.get());
        manager.setEmail(managerDto.getEmail());
        manager.setPassword(passwordEncoder.encode(managerDto.getPassword()));
        manager.setRoles(Collections.singleton(roleRepository.findByRoleName(RoleName.FILIAL_MANAGER)));
    ...
    public Boolean sendEMail(String emailCode, String sendingEmail){
        try {
            ...
            mailMessage.setText("<a href='http://localhost:8080/api/auth/verifyEmail/filialManager?emailCode="+emailCode+"&email="+sendingEmail+"'>Tasdiqlang</a>");
